...
import { MessagesContext, MessagesStateType, MessageType } from '@/components/notifications/NotificationProvider';
...
export default function useNotification() {
    ...
    const addMessage = (message: MessageType) => {
        ...
        dispatch((prevState: MessagesStateType) => { return {...prevState, messages: [...prevState.messages, message], nextId: prevState.nextId + 1 }});
    };
    ...
    const setDeleteMessageTimeout = (id: number) => {
        ...
        setTimeout(() => {
            ...
            dispatch((prevState: MessagesStateType) => { 
                return {...prevState, messages: prevState.messages.filter(m => m.id != id) };
            });
        },
        MESSAGE_SHOWTIME);
    };
    ...
    return {
        ...
        showInfoMessage: (info: string): void => {
            ...
            const message: MessageType = { message: info, status: "info", id: state.nextId };
        ...
        showErrorMessage: (info: string): void => {
            ...
            const message: MessageType = { message: info, status: "error", id: state.nextId };
        ...
        showMessage: (info: string, type: "info" | "error" | "warning" | "success"): void => {

            const message: MessageType = { message: info, status: type, id: state.nextId };
        ...
        handleNetworkException: (error: Error): void => {
            ...
            const message: MessageType = { message: errorMessage, status: "error", id: state.nextId };
            addMessage(message);
            setDeleteMessageTimeout(message.id);
        },
    };
